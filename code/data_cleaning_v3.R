...
# How many NA values appear in the control group?
nrow(missing_values[which(missing_values$year == 2013 & missing_values$tariff_type == 'Std'),])
...
#NA ROWS
missing_values_ss <- hh_data_ss[which(rowSums(is.na(hh_data_ss[,3:50])) > 0),] # Subset of hh_data_ss, showing all the rows with more than 0 NA values.
...
# The reason you aren't finding any NAs is because all the NAs that appear in the full data set were dealt with
# earlier in the linear interpolation section! :P  You can see that missing_values_ss is a dataframe of 0
# observations.
...
hh_data_ss$year_month <- substr(ISOdate(as.numeric(as.character(hh_data_ss$year)),
                                        as.numeric(as.character(hh_data_ss$month)),1),1,10)
hh_data_ss$year_month <- factor(hh_data_ss$year_month)

# Comment: as mentioned in our call, potentially the above line creates a redundant feature given that the 
#          control group is only across 2013.
...
hh_data_ss$hourly_sd <- rowSds(hh_cons) # Removed na.rm = TRUE as all the NAs have been dealt with!
hh_data_ss$hourly_sdm_ratio <- hh_data_ss$hourly_sd/(hh_data_ss$daily_total/48)
rm(hh_cons)

#daily profile (max = 1)

norm_hh <- sweep(hh_data_ss[,3:50],hh_data_ss[,"hh_max"],MARGIN=1,"/") # Can you explain this line more Simon?
# Is hh_max of each row being substracted from all the half hourly periods? I presume you are wanting to 
# normalise the data. I thought the formula for normalisation is (value - mean)/standard deviation. Please
# let me know if I am misunderstanding you!

colN <- sprintf("phh_%s",(0:47)) # What do the phh_#s columns represent?
...
#' 
#' See link: https://www.edfenergy.com/sites/default/files/time_of_use_heating_tariffs.pdf 
#' 
...
save(hh_data_ss,ID_type,input_hh,output_hh_adv, output_hh_comf, output_hh_aff, file = "../Data/SML_data/tidy_data/hh_class_input.Rda")
save(ID_type,input_hh,output_hh_adv, output_hh_comf, output_hh_aff, file = "data/hh_class_input_short.Rda")
...
# Jacob: In my input I have 4074 rows. Hmmm.